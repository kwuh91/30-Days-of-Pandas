...
# "tweet_id"] for x in tweets.index if len(tweets.loc[x, "content"]) > 15]})
#
#
...
# 1]] Employee = pd.DataFrame(data, columns=['id', 'name', 'salary', 'departmentId']).astype( {'id': 'Int64',
# 'name': 'object', 'salary': 'Int64', 'departmentId': 'Int64'}) data = [[1, 'IT'], [2, 'Sales']] Department =
# pd.DataFrame(data, columns=['id', 'name']).astype({'id': 'Int64', 'name': 'object'})
#
#
...
#     hash: dict[int, dict:[str, int]] = {}  # hash[id][event_day] = time
#     for x in employees.index:
...
#     def compare_dates(date1: str, date2: str) -> bool:  # True - frist bigger False - second bigger
...
#     hash: dict[str, dict[str, [set, set]]] = {}  # hash[date][name][0] = leads
...
#     hash: dict[int, dict[str, dict[str, int]]] = {}  # hash[id][name][subject] = attended
